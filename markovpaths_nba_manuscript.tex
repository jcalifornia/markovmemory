...
\textbf{Example: }
As a concrete example, consider a model for free throw shooting informed using $J=2$ observed trajectories given: $\{\boldsymbol\xi^{(1)} = +-++-++, \xi^{(2)} =  +--+-+++++-\}$. Suppose that
we set $h=1$ in this model. This choice implies that we need counts $N_{++}^{}=6, N_{+-}=5, N_{--}=1, N_{-+}=4$ coinciding to the number of makes following makes, misses following makes, misses following misses and makes following misses respectively for each trajectory. In addition, since the first state in each trajectory is stochastic as well, we add two special states representing the outcome of
the initial free throw, $N_{\cdot -} = 0$, $N_{\cdot +} = 2$. In the vector notation above, we have $\mathbf{N}_+ = [5,6], \mathbf{N}_-=[1,4], \mathbf{N}_\cdot = [0,2]$, where the indices 
