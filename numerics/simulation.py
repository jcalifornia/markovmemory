...
def fit_jagged_model(trajectories, states, prestates = [''], alpha = 1):
    """
    Fit jagged model with given prestates. Any transition not found in prestates
    is independent. Assume that prestates do not have more than one leading 0
    :param trajectories:
    :param states:
    :param prestates:
    :param alpha:
    ...
    max_q = max(1,max(len(p) for p in prestates))
    print(max_q)

    start_prestates = []
    for prestate in prestates:
        if prestate[0] == '0':
            start_prestates.append(prestate)
            prestates.remove(prestate)

    prestates0 = sorted(prestates, key = lambda x: len(x),reverse = True)
...
def evaluate_jagged_model(trajectories, prestates, states, alpha):
    """
    Compute model criteria for given jagged model
    :param trajectories:
    :param model:
    :param alpha:
    :return:
    ...
    # print(f"Fitting models of {q} states of hysteresis")
    N = defaultdict(partial(np.zeros, len(states)))
    N2 = defaultdict(partial(np.zeros, len(states)))  # sums for the first half of trajectories
    ...
    # collect the prestates, finding values of q
