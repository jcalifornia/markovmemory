...
\begin{document}
\title[Memory determination in finite-state random walks]{Bayesian predictive selection of memory for multistep Markov chains: LeBron James' three point shooting in the 2017 NBA playoffs}
...
\begin{abstract}
Consider the problem of modeling memory for discrete-state random walks using higher-order Markov chains.  This Letter introduces a Bayesian framework under the principle of predictive accuracy to select, from data, the number of prior states of recent history upon which a trajectory is statistically dependent. In this framework, I provide closed-form expressions for several alternative model selection criteria that approximate model prediction error for future data. Using simulations, I evaluate the prediction error of these criteria. 
...
A simple real-world application of this method is the evaluation of ``hot-hand phenomenon'' in the context of basketball jump-shooting.   In the 2017 NBA playoffs, the Cleveland Cavaliers played a total of 18 games. In these games, LeBron James attempted $96$ three pointers, making $42$ of them ($44\%$). Taking the outcome of each shot as a state in the set $\{$made, missed$\}$, and the shots in each game as a trajectory, one may evaluate memory effects in James' three-point shooting.
