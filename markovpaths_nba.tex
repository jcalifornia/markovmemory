...
\title[Memory determination in finite-state random walks]{Evaluating the hot hand phenomenon using predictive memory selection for multistep Markov Chains}
...
This Letter infers factorized probability representations for discrete-state stochastic paths, determining observationally whether and how transitions  are  dependent on memory. Assume that a trajectory $\xi$ consists of steps $\xi_l$, where each step takes  a value $x_l$ taken from the set $\{1,2,\ldots,M\}.$ We are interested in representations for the trajectory probability of the  form
%\begin{align}
...
\caption{\textbf{Hybrid jagged model} for free throw outcome where outcomes are independent except immediately after a missed free throw. AIC: $869.40$, WAIC$_1: 869.45$ , WAIC$_2: 869.52$ , LOO: $869.52$} 
\label{fig:fig5}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

This Letter has shown that LOO and WAIC can learn from data the physical reality of the 
